    ...
    class Error
    {
        public void ListaError(int opc,String reservada, int linea) ///nota: el parametro [reservada] llegan varibales o palabras reservadas
        {
            switch (opc)
            {
               ...
               case 7: { Console.WriteLine("Error de asignacion en la varibale (" + reservada + ") No es compatible con el tipo de dato entero: Linea " + linea); Console.Read(); } break;
               case 8: { Console.WriteLine("Error de asignacion en la varibale ("+reservada+") No es compatible con el tipo de dato cadena: Linea "+linea);Console.Read(); }break;
